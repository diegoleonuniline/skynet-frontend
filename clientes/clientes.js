      ...
      <tr>
        <td>
          <div class="table__user">
            <div class="table__avatar table__avatar--${colores[i % colores.length]}">
        ...
        <td><span class="badge badge--${c.estado}">${c.estado?.toUpperCase() || 'ACTIVE'}</span></td>
        <td class="font-semibold">${formatoMoneda(c.cuota_mensual)}</td>
        <td>
          <div class="table__actions">
            <button class="table__action-btn" onclick="verCliente('${c.id}')" title="View">
              <span class="material-symbols-outlined">visibility</span>
            </button>
            <button class="table__action-btn" onclick="editarCliente('${c.id}')" title="Edit">
              <span class="material-symbols-outlined">edit</span>
            </button>
            <button class="table__action-btn" title="More">
...
function abrirModalNuevo() {
  document.getElementById('modalTitulo').textContent = 'Register New Client';
  document.getElementById('formCliente').reset();
  document.getElementById('clienteId').value = '';
  document.getElementById('fechaInstalacion').value = new Date().toISOString().split('T')[0];
  document.getElementById('coloniaId').innerHTML = '<option value="">Select city first...</option>';
...
async function editarCliente(id) {
  try {
    ...
    document.getElementById('modalTitulo').textContent = 'Edit Client';
...
async function guardarCliente() {
  ...
  if (!datos.nombre || !datos.telefono || !datos.ciudad_id || !datos.colonia_id || !datos.direccion || !datos.plan_id) {
    toastAdvertencia('Please fill all required fields');
    return;
  }
  ...
  try {
    const data = id 
      ? await fetchPut(`/api/clientes/${id}`, datos) 
      : await fetchPost('/api/clientes', datos);
    ...
    if (data.ok) {
      cerrarModal();
      cargarClientes();
      toastExito(id ? 'Client updated successfully' : 'Client created successfully');
    } else {
      toastError(data.mensaje || 'Error saving client');
    }
  } catch (err) {
    console.error('Error:', err);
    btnLoading(btn, false);
    toastError('Error saving client');
  }
}
...
async function guardarCiudad() {
  ...
  if (!nombre) {
    toastAdvertencia('City name is required');
    return;
  }
  ...
  try {
    ...
    if (data.ok) {
      ...
      toastExito('City created');
    } else {
      toastError(data.mensaje || 'Error');
    }
  } catch (err) {
    btnLoading(btn, false);
    toastError('Error creating city');
  }
}
...
async function guardarColonia() {
  ...
  if (!ciudadId || !nombre) {
    toastAdvertencia('City and name are required');
    return;
  }
  ...
  try {
    ...
    if (data.ok) {
      ...
      toastExito('Neighborhood created');
    } else {
      toastError(data.mensaje || 'Error');
    }
  } catch (err) {
    btnLoading(btn, false);
    toastError('Error creating neighborhood');
  }
}
...
async function guardarPlan() {
  ...
  if (!nombre || !velocidad || !precio) {
    toastAdvertencia('All fields are required');
    return;
  }
  ...
  try {
    ...
    if (data.ok) {
      ...
      toastExito('Plan created');
    } else {
      toastError(data.mensaje || 'Error');
    }
  } catch (err) {
    btnLoading(btn, false);
    toastError('Error creating plan');
  }
}
